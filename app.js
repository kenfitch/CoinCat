...
const App = (() => {
  ...
  function transformAndStore(side, base64, rotateSteps = 0) {
    ...
    const img = new Image();
    img.onload = () => {
      // Simple, high-quality approach: just resize to fit the circle nicely
      const canvas = document.createElement("canvas");
      const ctx = canvas.getContext("2d");
      
      // High resolution for quality (will display at 120px but store at 400px for clarity)
      const finalSize = 400;
      canvas.width = finalSize;
      canvas.height = finalSize;
      
      // Calculate dimensions to cover the circle with some zoom for better fill
      const sourceRatio = img.width / img.height;
      const zoomFactor = 1.3; // Zoom in 30% to ensure coins fill the circle
      let drawWidth, drawHeight, offsetX, offsetY;
      
      if (sourceRatio > 1) {
        // Image is wider - fit by height
        drawHeight = finalSize * zoomFactor;
        drawWidth = finalSize * sourceRatio * zoomFactor;
        offsetX = -(drawWidth - finalSize) / 2;
        offsetY = -(drawHeight - finalSize) / 2;
      } else {
        // Image is taller - fit by width  
        drawWidth = finalSize * zoomFactor;
        drawHeight = (finalSize / sourceRatio) * zoomFactor;
        offsetX = -(drawWidth - finalSize) / 2;
        offsetY = -(drawHeight - finalSize) / 2;
      }
